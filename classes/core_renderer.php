...
class theme_morecandy_core_renderer extends theme_bootstrapbase_core_renderer {
    ...
    public function home_link() {
        ...
        if ($this->page->pagetype == 'site-index') {
            ...
            return html_writer::start_tag('div', array('class' => 'sitelink')) .
                   html_writer::link('http://docs.moodle.org/en/Administrator_documentation',
                   html_writer::empty_tag('img',
                   array('style' => 'width:100px;height:30px;', 'src' => $this->pix_url('moodlelogo'), 'alt' => 'moodlelogo')),
                   array('title' => 'Moodle', 'onclick' => 'this.target=_blank')).
                   html_writer::end_tag('div');

        } else if ($this->page->course->id == $SITE->id || strpos($this->page->pagetype, 'course-view') === 0) {
            return html_writer::tag('div',
                   html_writer::link($CFG->wwwroot,
                   html_writer::tag('i', '', array('class' => 'icon-home')) . '&nbsp;' . get_string('home') .
                   array('class' => 'btn btn-small')),
                   array('class' => 'homelink'));
        } else {
            return html_writer::tag('div',
                   html_writer::link($CFG->wwwroot . '/course/view.php?id=' . $this->page->course->id ,
        ...
        if (empty($course->id)) {
            ...
            if (isguestuser()) {
                ...
                if (!$loginpage && $withlinks) {
                    $loggedinas .= '<a class="btn btn-small btn-default" href="' . $loginurl . '">
                    <i class="fa fa-sign-in"></i> ' . get_string('login') . '</a>';
                }
                ...
                if ($withlinks) {
                    $loggedinas .= ' (<a href="$CFG->wwwroot/course/view.php?id=$course->id&amp;switchrole=0&amp;sesskey='.
                ...
                if (!isguestuser()) {
                    ...
                    if ($count = user_count_login_failures($USER)) {
                        ...
                        if (file_exists("$CFG->dirroot/report/log/index.php")
                            and has_capability('report/log:view', context_system::instance())) {
